...
public:
  ButtonFrame(WideString text, Frame* parent, int id = 0, int style = BS_PUSHBUTTON);
...
class LinkFrame : public WindowFrame
{
  ...
  uint32 onMessage(uint32 message, uint32 wParam, uint32 lParam);
public:
  LinkFrame(WideString text, Frame* parent, int id = 0);
...
public:
  ...
  void reset();
  int addString(WideString text, int data = 0);
...
public:
  ...
  StaticFrame(WideString text, Frame* parent, int id = 0, int style = 0, int exStyle = 0);
...
public:
  ...
  void setRichText(WideString text);
};
...
public:
  ...
  Frame* addTab(WideString text, Frame* frame = NULL);
...
public:
  ...
  void setItemText(HTREEITEM item, WideString text);
};
